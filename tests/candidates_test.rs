...
fn binance_map_standard_coins() {
    for (hl, bn) in [("BTC","BTCUSDT"),("ETH","ETHUSDT"),("SOL","SOLUSDT"),("AVAX","AVAXUSDT")] {
...
fn binance_map_k_prefix_becomes_1000() {
    for (hl, bn) in [("kBONK","1000BONKUSDT"),("kPEPE","1000PEPEUSDT"),("kSHIB","1000SHIBUSDT")] {
        assert_eq!(hl_to_binance(hl), Some(bn.to_string()));
    }
}
...
fn binance_map_rejects_at_and_slash() {
    for invalid in ["@232", "@7", "@1000", "PURR/USDC", "BTC/USDC"] {
        assert_eq!(hl_to_binance(invalid), None, "{invalid} should have no Binance mapping");
    }
}
...
fn candidates_contains_anchors() {
    let c = mids(&[("BTC",50000.0),("ETH",3000.0),("SOL",100.0),("DOGE",0.1)]);
    let p = c.clone();
    let r = filter_candidates(&c, &p);
    for anchor in ["BTC","ETH","SOL"] {
        assert!(r.contains(&anchor.to_string()), "{anchor} anchor missing from candidates");
    }
}
...
fn candidates_capped_at_18() {
    let current: HashMap<String,f64> = (0..50)
        ...
        .chain([("BTC".into(),50000.0),("ETH".into(),3000.0),("SOL".into(),100.0)])
        .collect();
...
fn candidates_excludes_at_symbols() {
    ...
    let c = mids(&[("BTC",50000.0),("ETH",3000.0),("SOL",100.0),("@232",5000.0),("@7",7000.0)]);
    let p = mids(&[("BTC",50000.0),("ETH",3000.0),("SOL",100.0),("@232",1.0),("@7",1.0)]);
    let r = filter_candidates(&c, &p);
    assert!(!r.contains(&"@232".to_string()), "@232 must not be a candidate");
    assert!(!r.contains(&"@7".to_string()),   "@7 must not be a candidate");
}

#[test]
fn candidates_top_mover_selected_over_flat_coin() {
    let c = mids(&[("BTC",50000.0),("ETH",3000.0),("SOL",100.0),("MOON",2.0),("FLAT",1.0)]);
    let p = mids(&[("BTC",50000.0),("ETH",3000.0),("SOL",100.0),("MOON",1.0),("FLAT",1.0)]);
    let r = filter_candidates(&c, &p);
    assert!(r.contains(&"MOON".to_string()), "Top mover MOON should be included");
}
...
fn candidates_handles_empty_previous_gracefully() {
    ...
    let r = filter_candidates(&c, &p);
    assert!(r.contains(&"BTC".to_string()), "BTC anchor must survive empty prev map");
}
...
fn candidates_k_prefix_tokens_pass_binance_filter() {
    ...
    let c = mids(&[("BTC",50000.0),("ETH",3000.0),("SOL",100.0),("kBONK",0.002),("kPEPE",0.00001)]);
    let p = mids(&[("BTC",50000.0),("ETH",3000.0),("SOL",100.0),("kBONK",0.001),("kPEPE",0.000005)]);
    let r = filter_candidates(&c, &p);
    assert!(r.contains(&"kBONK".to_string()), "kBONK should be a valid candidate");
    assert!(r.contains(&"kPEPE".to_string()), "kPEPE should be a valid candidate");
}
...
fn sentiment_kprefix_bug_regression_kbonk_resolves_to_bonk() {
    ...
    assert_eq!(lc_key, "BONK", "kBONK must normalise to BONK for LunarCrush lookup");
    ...
    assert!(hit.is_some(), "Cache lookup with normalised key must succeed");
...
fn sentiment_normalise_all_known_k_prefix_tokens() {
    ...
    for (hl, lc) in cases {
        assert_eq!(normalise_for_lunarcrush(hl), lc,
...
fn sentiment_normalise_preserves_uppercase_k_names() {
    ...
    for sym in ["KAVA", "KLAY", "KSM"] {
        assert_eq!(normalise_for_lunarcrush(sym), sym,
...
fn pipeline_candidate_to_sentiment_lookup_end_to_end() {
    ...
    let current = mids(&[
        ("BTC",50000.0), ("ETH",3000.0), ("SOL",100.0),
...
fn pipeline_at_symbols_never_reach_sentiment_lookup() {
    ...
    let current = mids(&[("BTC",50000.0),("ETH",3000.0),("SOL",100.0),("@232",5000.0)]);
    let previous = mids(&[("BTC",50000.0),("ETH",3000.0),("SOL",100.0),("@232",1.0)]);

    let candidates = filter_candidates(&current, &previous);

    // @232 must not be in candidates, so sentiment for it is never queried
    assert!(!candidates.contains(&"@232".to_string()),
