...
fn detect_order_flow(bid_vol: f64, ask_vol: f64) -> OrderFlowResult {
    ...
    OrderFlowResult { direction, confidence, imbalance_ratio: ratio }
}

// ── Signal weights helpers ────────────────────────────────────────────────────

struct SignalWeightsSimple {
    rsi: f64, bollinger: f64, macd: f64, ema_cross: f64,
    order_flow: f64, z_score: f64, volume: f64, sentiment: f64,
    funding_rate: f64, trend: f64,
}

impl SignalWeightsSimple {
    fn default_weights() -> Self {
        SignalWeightsSimple {
            rsi: 0.17, bollinger: 0.14, macd: 0.07, ema_cross: 0.07,
            order_flow: 0.14, z_score: 0.17, volume: 0.03, sentiment: 0.10,
            funding_rate: 0.08, trend: 0.03,
        }
    }

    fn sum(&self) -> f64 {
        self.rsi + self.bollinger + self.macd + self.ema_cross
          + self.order_flow + self.z_score + self.volume + self.sentiment
          + self.funding_rate + self.trend
    }
...
fn order_flow_confidence_symmetry_tier1() {
    ...
    assert_eq!(short_sig.confidence, 0.95,
...
fn order_flow_confidence_symmetry_tier2() {
    ...
    assert_eq!(short_sig.confidence, 0.85,
...
fn order_flow_confidence_symmetry_tier3() {
    ...
    assert_eq!(long_sig.confidence, short_sig.confidence,
        ...
        long_sig.confidence, short_sig.confidence);
    assert_eq!(short_sig.confidence, 0.70,
...
fn signal_weights_default_sum_to_one() {
    ...
    let sum = w.sum();
    assert!((sum - 1.0).abs() < 1e-6, "default weights must sum to 1.0, got {sum:.8}");
}

#[test]
fn signal_weights_normalise_preserves_proportions() {
    let mut w = SignalWeightsSimple {
        rsi: 2.0, bollinger: 2.0, macd: 1.0, ema_cross: 1.0,
        order_flow: 2.0, z_score: 2.0, volume: 0.5, sentiment: 1.5,
        funding_rate: 1.0, trend: 0.5,
    };
    ...
    assert!((w.sum() - 1.0).abs() < 1e-6,
        "after normalise, sum must be 1.0, got {:.8}", w.sum());
}

#[test]
fn signal_weights_all_zero_no_panic() {
    let mut w = SignalWeightsSimple {
        rsi: 0.0, bollinger: 0.0, macd: 0.0, ema_cross: 0.0,
        order_flow: 0.0, z_score: 0.0, volume: 0.0, sentiment: 0.0,
        funding_rate: 0.0, trend: 0.0,
    };
...
fn classify_regime_with_override(adx: f64, atr_expansion: f64) -> &'static str {
    ...
    let base = if adx > 27.0 { "Trending" } else if adx < 19.0 { "Ranging" } else { "Neutral" };
...
fn regime_atr_override_ranging_to_trending() {
    ...
    assert_eq!(regime, "Trending", "Ranging + ATR expansion → override to Trending");
}
...
fn ema_cross_long_short_score_parity() {
    ...
    let bull = if ema_pct_bull > 0.5 { w } else if ema_pct_bull > 0.2 { w * 0.6 } else { 0.0 };
    let bear = if ema_pct_bear < -0.5 { w } else if ema_pct_bear < -0.2 { w * 0.6 } else { 0.0 };
...
fn funding_signal_strength(rate: f64) -> f64 {
    ...
    if      rate >  0.0010 { -1.00 }
    else if rate >  0.0005 { -0.65 }
    else if rate >  0.0002 { -0.30 }
    else if rate < -0.0010 {  1.00 }
    else if rate < -0.0005 {  0.65 }
    else if rate < -0.0002 {  0.30 }
    else                   {  0.00 }
}
...
fn funding_extreme_long_crowding_is_bearish() {
    ...
    assert_eq!(s, -1.00, "extreme positive funding (longs crowded) → -1.0 (bearish)");
}

#[test]
fn funding_extreme_short_crowding_is_bullish() {
    let s = funding_signal_strength(-0.0015);
    assert_eq!(s, 1.00, "extreme negative funding (shorts crowded) → +1.0 (bullish)");
}
...
fn funding_neutral_band_is_zero() {
    for rate in [0.0001_f64, -0.0001, 0.0, 0.00015, -0.00015] {
        ...
        assert_eq!(s, 0.00, "rate {rate:.5} is inside neutral band → 0.0 signal");
    }
}
...
fn funding_signal_long_short_symmetry() {
    for (pos, neg) in [(0.0003, -0.0003), (0.0006, -0.0006), (0.0011, -0.0011)] {
        ...
        assert_eq!(s_pos.signum(), -s_neg.signum(),
...
fn btc_context_low_dominance_always_zero() {
    for ret in [-5.0_f64, -1.0, 0.0, 1.0, 5.0] {
        for action in ["BUY", "SELL"] {
            ...
            assert_eq!(adj, 0.0, "dominance <48% → 0 adj (altseason): ret={ret}, act={action}");
        }
    }
}
...
fn bull_bear_scores_always_non_negative() {
    ...
    for rsi in [10.0_f64, 30.0, 50.0, 70.0, 90.0] {
        for z in [-3.0_f64, -1.5, 0.0, 1.5, 3.0] {
            ...
            // RSI (Ranging)
            if rsi < 28.0      { bull += weights.rsi; }
            else if rsi > 72.0 { bear += weights.rsi; }
            else if rsi < 40.0 { bull += weights.rsi * 0.60; }
            else if rsi > 60.0 { bear += weights.rsi * 0.60; }

            // Z-score (Ranging PRIMARY)
            let zw = weights.z_score * 1.40;
            if z < -2.0      { bull += zw; }
            else if z > 2.0  { bear += zw; }
            else if z < -1.4 { bull += zw * 0.60; }
            else if z > 1.4  { bear += zw * 0.60; }

            assert!(bull >= 0.0, "bull score must be ≥ 0: rsi={rsi}, z={z}, bull={bull}");
            assert!(bear >= 0.0, "bear score must be ≥ 0: rsi={rsi}, z={z}, bear={bear}");
        }
    }
}
...
fn learner_weight_update_preserves_sum_to_one() {
    ...
    assert!(rsi + ema <= 1.0 + 1e-6,
        "after 5 updates, rsi+ema should not exceed 1.0: {:.6}", rsi + ema);
